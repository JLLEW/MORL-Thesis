...
class MetricsLogger:
    ...
    def gather_metrics(self, action, rewards):
        ...
        #self.profit_reward.append(rewards)
    ...
    def save_metrics(self):
        ...
        for data_name, data in self.env.data.items():
            df[f"{data_name} price"] = data.iloc[self.initial_data_i:, 0].tolist()
            df[f"{data_name} allocation"] = [x[i] for x in self.allocations]
            i += 1
        
        df = pd.DataFrame(df)
        df.to_csv(f"{self.path}/evaluation_metrics_backtest_validation.csv")
...
if __name__ == '__main__':
    ...
    if args.scenario == 'sp' or args.scenario == "sp_ex":
        _, market1_data, market2_data = utils.generate_data(args.scenario)
        if args.market == 1:
            data = market1_data
        else:
            data = market2_data
    else:
        _, data = utils.generate_data(args.scenario)
    assets = utils.get_assets_name_list(args.scenario)
    env = train_models.create_env(args.model_name, data, assets, args.ep_length, args.seed, True)
